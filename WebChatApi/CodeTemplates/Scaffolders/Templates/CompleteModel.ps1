...
$directorioProyecto=(get-item $PSScriptRoot).parent.parent.parent.parent.FullName
$directorioCompleto=$directorioProyecto + "\Dominio"
$items = Get-ChildItem -Path $directorioCompleto
foreach ($item in $items)
{
      ...
      {
          if($item.Name -ne "bin" -and $item.Name -ne "obj" -and $item.Name -ne "Properties" -and $item.Name -and "AttachmentDominio" -and $item.Name -ne "IdentificableObject" -and $item.Name -ne "Properties"-and $item.Name -eq "ChatRooms" )
			{
           $archivo=$directorioCompleto + "\"+$item.Name
			$files = Get-ChildItem -Path $archivo
...
Add-ProjectItemViaTemplate $outputPath -Template $templateName -Model @{
									...
									$namespaceController = "AdministracionInterna.Controllers"
